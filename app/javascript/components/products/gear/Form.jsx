...
import { GEAR_CATEGORIES, STATUSES, CONDITIONS } from "../shared/constants";
...
export default function GearForm({ product, isEditing = false }) {
  ...
  const onSubmit = async (data) => {
    try {
      ...
      const formData = {
        ...data,
        product_images_attributes: data.product_images_attributes
          .map((img) => ({
            id: img.id,
            title: img.title,
      ...
      if (isEditing) {
        response = await patch(
          `/${targetUsername}/products/gear/${slug}.json`,
          {
            responseKind: "json",
              ...
              <div className="block pt-0 space-y-3 md:col-span-2">
                    ...
                    <FormItem>
                      ...
                      <FormControl>
                        <Select
                          id="category"
                          placeholder={I18n.t(
                            "products.gear.form.select_category"
                          )}
                          options={GEAR_CATEGORIES}
                          value={GEAR_CATEGORIES.find(
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel>
                        {I18n.t("products.gear.form.status")}
                      </FormLabel>
                      <FormControl>
                        <Select
                          id="status"
                          placeholder={I18n.t(
                            "products.gear.form.select_status"
                          )}
                          options={STATUSES}
