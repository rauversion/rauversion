...
export default function TenantBilling() {
  ...
  if (loading) return <div className="grid min-h-[60vh] place-items-center"><Loader2 className="h-7 w-7 animate-spin text-emerald-400" /></div>
  ...
  return (
    <main className="relative min-h-screen overflow-hidden rounded-[2rem] border border-border/60 bg-zinc-950 px-5 py-8 text-zinc-50 sm:px-8 lg:px-12 lg:py-12">
      <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_12%_0%,rgba(52,211,153,.15),transparent_30%),radial-gradient(circle_at_90%_45%,rgba(34,211,238,.08),transparent_25%)]" />
      <div className="relative mx-auto max-w-6xl">
        <Button variant="ghost" asChild className="-ml-3 mb-6 text-zinc-400"><a href="/tenants"><ArrowLeft className="mr-2 h-4 w-4" /> Tenants</a></Button>
        <header className="flex flex-col justify-between gap-6 md:flex-row md:items-end">
          <div><div className="flex items-center gap-2 text-xs font-medium uppercase tracking-[0.2em] text-emerald-400"><Sparkles className="h-4 w-4" /> Plan del tenant</div><h1 className="mt-3 text-3xl font-semibold tracking-[-0.04em] sm:text-5xl">Activa {data.tenant.name}</h1><p className="mt-3 max-w-xl text-zinc-400">El plan controla acceso, límites y capacidades comerciales de este espacio.</p></div>
        ...
        {data.disabled && <Alert className="mt-8 border-amber-300/20 bg-amber-300/[0.06]"><ShieldCheck className="h-4 w-4 text-amber-300" /><AlertTitle>Suscripciones deshabilitadas</AlertTitle><AlertDescription>DISABLE_TENANT_SUBSCRIPTION está activo. El tenant tiene acceso sin cobro en este ambiente.</AlertDescription></Alert>}
        {data.tenant.central && <Alert className="mt-8 border-cyan-300/20 bg-cyan-300/[0.06]"><ShieldCheck className="h-4 w-4 text-cyan-300" /><AlertTitle>Tenant global</AlertTitle><AlertDescription>El tenant central está exento de suscripción.</AlertDescription></Alert>}
        {data.subscription && <Card className="mt-8 border-emerald-400/20 bg-emerald-400/[0.05]"><CardContent className="flex flex-col justify-between gap-4 p-6 sm:flex-row sm:items-center"><div><p className="text-xs uppercase tracking-[0.2em] text-emerald-400">Plan actual</p><p className="mt-2 text-xl font-semibold">{data.subscription.plan_name}</p><p className="mt-1 text-sm text-zinc-400">Estado: {data.subscription.status}</p></div><Badge className="w-fit bg-emerald-400 text-zinc-950">{data.accessible ? "Acceso habilitado" : "Pendiente"}</Badge></CardContent></Card>}
        ...
        <section className="mt-9 grid gap-5 lg:grid-cols-3">
          {data.plans.map((plan, index) => {
            ...
            return <Card key={plan.id} className={cn("relative overflow-hidden border-white/10 bg-white/[0.035]", index === 1 && "border-emerald-400/35 bg-emerald-400/[0.045]")}>
              {index === 1 && <div className="absolute inset-x-0 top-0 h-0.5 bg-gradient-to-r from-emerald-400 to-cyan-300" />}
              <CardHeader><div className="flex items-center justify-between"><CardTitle className="text-2xl">{plan.name}</CardTitle>{current && <Badge variant="outline">Actual</Badge>}</div><CardDescription className="min-h-10">{plan.description}</CardDescription>{price && <div className="pt-4"><span className="text-3xl font-semibold">{formatPrice(price)}</span><span className="text-sm text-zinc-500"> / {price.billing_interval === "year" ? "año" : "mes"}</span></div>}</CardHeader>
              <CardContent className="space-y-5"><div className="space-y-3">{Object.entries(plan.entitlements).filter(([key, value]) => key !== "tenant_access" && value !== false).slice(0, 7).map(([key, value]) => <div key={key} className="flex items-center gap-2 text-sm text-zinc-300"><span className="grid h-5 w-5 place-items-center rounded-full bg-emerald-400/10 text-emerald-400"><Check className="h-3 w-3" /></span><span>{typeof value === "number" ? `${value} ` : ""}{featureLabels[key] || key.replaceAll("_", " ")}</span></div>)}</div><Button className="w-full" disabled={data.disabled || data.tenant.central || current || !price?.checkout_available || submittingId !== null} onClick={() => price && checkout(price)}>{submittingId === price?.id ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <CreditCard className="mr-2 h-4 w-4" />}{current ? "Plan actual" : price?.checkout_available ? "Elegir plan" : "Configura Stripe Price"}</Button></CardContent>
            </Card>
          })}
        </section>
      </div>
