...
function BrandPreview({ tenant, branding, logoPreview }: { tenant: Tenant; branding: Branding; logoPreview: string | null }) {
  ...
  return (
    <div
      className={cn("relative isolate min-h-[31rem] overflow-hidden border border-white/10 p-7 shadow-2xl", templateClass)}
    ...
    >
      ...
      <div className="relative flex items-center justify-between gap-4">
        <div className="flex min-w-0 items-center gap-3">
          {logoPreview ? (
            ...
            <div className="grid h-11 w-11 place-items-center rounded-xl font-bold text-zinc-950" style={{ backgroundColor: branding.primary_color }}>
      ...
      <div className={cn("relative mt-24", branding.template === "editorial" && "mt-16 border-l pl-6")} style={{ borderColor: branding.accent_color }}>
        <Badge className="border-0 text-zinc-950" style={{ backgroundColor: branding.accent_color }}>Nueva temporada</Badge>
        ...
        <Button className="mt-7 border-0 text-zinc-950" style={{ backgroundColor: branding.primary_color }}>Explorar catálogo</Button>
      </div>
      ...
      <div className="absolute inset-x-7 bottom-7 grid grid-cols-3 gap-2">
        {["Último release", "Próximo evento", "Selección"].map((label, index) => (
          <div key={label} className="rounded-lg border border-white/10 bg-white/[0.04] p-3">
            <div className="mb-3 h-1 rounded-full bg-white/10"><div className="h-full rounded-full" style={{ width: `${82 - index * 18}%`, backgroundColor: index === 1 ? branding.accent_color : branding.primary_color }} /></div>
...
export default function TenantSettings() {
  ...
  return (
    <main className="relative min-h-screen overflow-hidden rounded-[2rem] border border-border/60 bg-zinc-950 px-4 py-6 text-zinc-50 sm:px-7 lg:px-10 lg:py-9">
      <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_0%_0%,rgba(52,211,153,.11),transparent_28%),radial-gradient(circle_at_100%_30%,rgba(34,211,238,.07),transparent_24%)]" />
      <div className="relative mx-auto max-w-7xl">
        <header className="mb-8 flex flex-col justify-between gap-5 md:flex-row md:items-end">
          <div>
            <Button variant="ghost" asChild className="-ml-3 mb-4 text-zinc-400 hover:text-white"><a href="/tenants"><ArrowLeft className="mr-2 h-4 w-4" /> Tenants</a></Button>
            <div className="flex items-center gap-2 text-xs font-medium uppercase tracking-[0.2em] text-emerald-400"><Sparkles className="h-4 w-4" /> Estudio de marca</div>
            <h1 className="mt-3 text-3xl font-semibold tracking-[-0.04em] sm:text-4xl">Configura {tenant.name}</h1>
            <p className="mt-2 max-w-2xl text-sm text-zinc-400">Define la identidad del tenant. Estos valores quedan disponibles para el storefront y sus futuras plantillas.</p>
          </div>
        ...
        <form onSubmit={save} className="grid gap-7 xl:grid-cols-[minmax(0,0.88fr)_minmax(30rem,1.12fr)]">
          <div className="space-y-5">
            <Card className="border-white/10 bg-white/[0.035]">
              <CardHeader><CardTitle className="flex items-center gap-2"><ImageIcon className="h-5 w-5 text-emerald-400" /> Identidad</CardTitle><CardDescription>Nombre, mensaje y logo principal.</CardDescription></CardHeader>
              <CardContent className="space-y-5">
                ...
                <div className="space-y-2">
                  ...
                  <label htmlFor="tenant-logo" className="flex cursor-pointer items-center gap-4 rounded-xl border border-dashed border-white/15 bg-black/20 p-4 transition hover:border-emerald-400/40">
                    <div className="grid h-12 w-12 place-items-center overflow-hidden rounded-xl bg-white/5">{logoPreview ? <img src={logoPreview} alt="Logo" className="h-full w-full object-contain" /> : <Upload className="h-5 w-5 text-zinc-500" />}</div>
                    <div><p className="text-sm font-medium">{logoFile?.name || "Subir logo"}</p><p className="mt-1 text-xs text-zinc-500">PNG, JPG, WebP o SVG. Recomendado 512 x 512.</p></div>
                  </label>
            ...
            <Card className="border-white/10 bg-white/[0.035]">
              <CardHeader><CardTitle className="flex items-center gap-2"><Palette className="h-5 w-5 text-cyan-300" /> Paleta</CardTitle><CardDescription>Colores base usados por botones, acentos y fondos.</CardDescription></CardHeader>
            ...
            <Card className="border-white/10 bg-white/[0.035]">
              <CardHeader><CardTitle className="flex items-center gap-2"><Type className="h-5 w-5 text-amber-300" /> Tipografía</CardTitle></CardHeader>
              <CardContent>
          ...
          <div className="space-y-5 xl:sticky xl:top-5 xl:self-start">
            <Card className="border-white/10 bg-white/[0.035]">
              ...
              <CardContent className="grid gap-3 sm:grid-cols-3">
                {templates.map((template) => (
                  <button key={template.key} type="button" onClick={() => updateBranding("template", template.key)} className={cn("relative rounded-xl border p-4 text-left transition", branding.template === template.key ? "border-emerald-400/50 bg-emerald-400/[0.08]" : "border-white/10 bg-black/15 hover:border-white/20")}>
                    {branding.template === template.key && <span className="absolute right-3 top-3 grid h-5 w-5 place-items-center rounded-full bg-emerald-400 text-zinc-950"><Check className="h-3 w-3" /></span>}
                    <p className="text-sm font-semibold">{template.name}</p><p className="mt-2 text-xs leading-relaxed text-zinc-500">{template.description}</p>
                  </button>
                ))}
              </CardContent>
            </Card>
            ...
            <div className="flex items-center justify-between gap-4 rounded-2xl border border-white/10 bg-zinc-900/90 p-4 shadow-xl backdrop-blur">
              <div><p className="text-sm font-medium">{saved ? "Cambios guardados" : "Configuración del tenant"}</p><p className="mt-1 text-xs text-zinc-500">La vista previa se actualiza antes de publicar.</p></div>
              <Button type="submit" disabled={saving || !tenant.can_manage_settings} className="bg-emerald-400 text-zinc-950 hover:bg-emerald-300">{saving ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Save className="mr-2 h-4 w-4" />} Guardar</Button>
            </div>
          </div>
        </form>
      </div>
