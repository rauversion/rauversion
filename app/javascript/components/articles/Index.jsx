...
function ArticleCard({ article, featured = false }) {
  return (
    <motion.div
      ...
      className={`group bg-default border border-default/10 rounded-xl overflow-hidden hover:border-default/30 transition-all duration-300 ${featured ? 'col-span-2 row-span-2' : ''
        }`}
    >
      {/* Image Container */}
      ...
      <div className="p-6 space-y-5">
        <Link to={`/articles/${article.slug}`}>
          {/* Title */}
          <h3 className={`tracking-tight text-default font-semibold line-clamp-2 ${featured ? 'text-3xl md:text-4xl' : 'text-xl'
            }`}>
            {article.title}
          </h3>
