...
class VideoGenerator
  ...
  def generate!
    ...
    ffmpeg_cmd += ["-ss", audio_start.to_s] if audio_start.present?
    ffmpeg_cmd += ["-to", audio_end.to_s] if audio_end.present?
    ...
    Rails.logger.info(
      "VideoGenerator starting cover_image=#{cover_image} audio_file=#{audio_file} " \
      "mask_image=#{mask_image} output_file=#{output_file} duration=#{duration} " \
      "audio_start=#{audio_start.inspect} audio_end=#{audio_end.inspect} color=#{color} " \
      "disc_size=#{disc_size} loop_speed=#{loop_speed}"
    )
    Rails.logger.info("VideoGenerator ffmpeg command=#{Shellwords.join(ffmpeg_cmd)}")
    success = system(*ffmpeg_cmd)
    Rails.logger.info(
      "VideoGenerator ffmpeg finished success=#{success} output_exists=#{File.exist?(output_file)} " \
      "output_size=#{File.exist?(output_file) ? File.size(output_file) : 0}"
    )
    raise "ffmpeg failed to generate video" unless success && File.exist?(output_file)
  end
end
