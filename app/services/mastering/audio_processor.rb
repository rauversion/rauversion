...
module Mastering
  class AudioProcessor
    ...
    def max_positive_gain_db(current_lufs, limiter)
      ...
      profile_cap = number(policy["max_positive_gain_db"], 10.0)

      return [profile_cap, number(policy["loud_source_gain_cap_db"], 1.5)].min if current_lufs >= -10.0
    ...
    def true_peak_safety_margin_db
      number(mastering_policy["true_peak_safety_margin_db"], 0.5)
    end
    ...
    def mastering_policy
      recipe["mastering_policy"].presence || {
        "max_positive_gain_db" => target_profile.max_positive_gain_db,
        "loud_source_gain_cap_db" => target_profile.loud_source_gain_cap_db,
        "true_peak_safety_margin_db" => target_profile.true_peak_safety_margin_db
      }
    end

    def target_profile
      Mastering::TargetProfiles.fetch(recipe.dig("target", "profile"))
    end
